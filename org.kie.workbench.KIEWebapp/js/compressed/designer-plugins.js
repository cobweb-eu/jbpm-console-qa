...
}if(!g[l]&&l!="Connecting Objects"){if(Ext.isIE){g[l]=new Ext.tree.TreeNode({text:k,allowDrag:false,allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRepChild",singleClickExpand:true,expanded:true});
g[l].expand()
...
var E=jsonPath(f.evalJSON(),"$.properties.vardefs");
var m=new Array();
var p=new Array();
var d=new Hash();
var j=new Array();
...
var t=new Array();
p.push("");
p.push("** Variable Definitions **");
m.push(p);
j.push(p);
if(E){E.forEach(function(M){if(M.length>0){var J=M.split(",");
for(var L=0;
L<J.length;
L++){var K=new Array();
var N=J[L];
if(N.indexOf(":")>0){var I=N.split(":");
K.push(I[0]);
K.push(I[0]);
d[I[0]]=I[1];
b.push(I[0])
...
m.push(q);
z.push(q);
Ext.each(this.dataSource.data.items,function(L){if((L.data.gridProperties.propId=="oryx-datainputset")||(L.data.gridProperties.propId=="oryx-datainput")){var I=L.data.value.split(",");
for(var K=0;
K<I.length;
K++){var M=I[K];
var J=new Array();
if(M.indexOf(":")>0){var i=M.split(":");
J.push(i[0]);
J.push(i[0]);
d[i[0]]=i[1];
l.push(i[0])
...
m.push(s);
o.push(s);
Ext.each(this.dataSource.data.items,function(L){if((L.data.gridProperties.propId=="oryx-dataoutputset")||(L.data.gridProperties.propId=="oryx-dataoutput")){var J=L.data.value.split(",");
for(var i=0;
i<J.length;
i++){var M=J[i];
var K=new Array();
...
var G=new Ext.data.Store({autoDestroy:true,reader:new Ext.data.JsonReader({root:"root"},e),proxy:C,sorters:[{property:"atype",direction:"ASC"},{property:"from",direction:"ASC"},{property:"to",direction:"ASC"},{property:"tostr",direction:"ASC"}]});
G.load();
if(this.value.length>0){var w=this.value.split(",");
for(var B=0;
B<w.length;
B++){var g=w[B];
if(g.indexOf("=")>0){var y=g.split("=");
var A=d[y[0]];
if(!A){A="java.lang.String"
}var r=y[0];
y.shift();
var h=y.join("=").replace(/\#\#/g,",");
h=h.replace(/\|\|/g,"=");
if(b.indexOf(r)<0){G.add(new e({atype:(l.indexOf(r)>=0)?"DataInput":"DataOutput",from:r,type:"is equal to",to:"",tostr:h,dataType:A}))
}}else{if(g.indexOf("->")>0){var y=g.split("->");
var A=d[y[0]];
if(!A){A="java.lang.String"
}var r=y[0];
var H=false;
if(l.indexOf(r)>=0&&l.indexOf(y[1])>=0){if(b.indexOf(r)<0){H=true
}}if(l.indexOf(r)>=0&&b.indexOf(y[1])>=0){if(b.indexOf(r)<0){H=true
}}if(b.indexOf(r)>=0&&b.indexOf(y[1])>=0){if(l.indexOf(y[1])<0&&t.indexOf(y[1])){H=true
}}if(t.indexOf(r)>=0&&l.indexOf(y[1])>=0){H=true
}if(!H){var k="";
if(b.indexOf(r)>=0){if(t.indexOf(r)>=0){k="DataOutput"
}else{k="DataInput"
}}else{if(l.indexOf(r)>=0){if(t.indexOf(r)>=0){k="DataOutput"
}else{k="DataInput"
}}else{k="DataOutput"
}}G.add(new e({atype:k,from:y[0],type:"is mapped to",to:y[1],tostr:"",dataType:A}))
}}else{var A=d[g];
if(!A){A="java.lang.String"
}if(b.indexOf(g)<0){G.add(new e({atype:(b.indexOf(g)>=0||l.indexOf(g)>=0)?"DataInput":"DataOutput",from:g,type:"is equal to",to:"",tostr:"",dataType:A}))
}}}}}G.on("update",function(J,i,I){if(I=="edit"){var K=d[i.get("from")];
if(!K){K="java.lang.String"
}i.set("dataType",K)
}});
var F=new Ext.form.ComboBox({name:"fromCombo",valueField:"name",displayField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,store:new Ext.data.SimpleStore({fields:["name","value"],data:m})});
...
var x=Ext.id();
var a=new Ext.grid.EditorGridPanel({autoScroll:true,autoHeight:true,store:G,id:x,stripeRows:true,cm:new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),{id:"valueType",header:ORYX.I18N.PropertyWindow.dataType,width:180,dataIndex:"dataType",hidden:"true"},{id:"atype",header:"Assignment Type",width:180,dataIndex:"atype"},{id:"from",header:ORYX.I18N.PropertyWindow.fromObject,width:180,dataIndex:"from",editor:F},{id:"type",header:ORYX.I18N.PropertyWindow.assignmentType,width:100,dataIndex:"type",editor:v},{id:"to",header:ORYX.I18N.PropertyWindow.toObject,width:180,dataIndex:"to",editor:n},{id:"tostr",header:ORYX.I18N.PropertyWindow.toValue,width:180,dataIndex:"tostr",editor:new Ext.form.TextField({allowBlank:true}),renderer:Ext.util.Format.htmlEncode},u]),selModel:u,autoHeight:true,tbar:[{text:"[ New Data Input Assignment ]",handler:function(){G.add(new e({atype:"DataInput",from:"",type:"",to:"",tostr:""}));
c="datainput";
a.fireEvent("cellclick",a,G.getCount()-1,1,null)
}},{text:"[ New Data Output Assignment ]",handler:function(){G.add(new e({atype:"DataOutput",from:"",type:"",to:"",tostr:""}));
c="dataoutput";
a.fireEvent("cellclick",a,G.getCount()-1,1,null)
}}],clicksToEdit:1,listeners:{beforeedit:function(L){if(L.record.data.atype=="DataInput"){var i=L.grid.getColumnModel().getCellEditor(L.column,L.row)||{};
i=i.field||{};
if(i.name=="typeCombo"){i.destroy();
var I=new Ext.form.ComboBox({name:"typeCombo",valueField:"name",displayField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,store:new Ext.data.SimpleStore({fields:["name","value"],data:[["is mapped to",ORYX.I18N.PropertyWindow.isMappedTo],["is equal to",ORYX.I18N.PropertyWindow.isEqualTo]]})});
L.grid.getColumnModel().setEditor(L.column,new Ext.grid.GridEditor(I))
}if(i.name=="fromCombo"){i.destroy();
var J=new Ext.form.ComboBox({name:"fromCombo",valueField:"name",displayField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,store:new Ext.data.SimpleStore({fields:["name","value"],data:j.concat(z)})});
L.grid.getColumnModel().setEditor(L.column,new Ext.grid.GridEditor(J))
}if(i.name=="toCombo"){i.destroy();
var K=new Ext.form.ComboBox({name:"toCombo",valueField:"name",displayField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,store:new Ext.data.SimpleStore({fields:["name","value"],data:z})});
L.grid.getColumnModel().setEditor(L.column,new Ext.grid.GridEditor(K))
}}if(L.record.data.atype=="DataOutput"){var i=L.grid.getColumnModel().getCellEditor(L.column,L.row)||{};
i=i.field||{};
if(i.name=="typeCombo"){i.destroy();
var I=new Ext.form.ComboBox({name:"typeCombo",valueField:"name",displayField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,store:new Ext.data.SimpleStore({fields:["name","value"],data:[["is mapped to",ORYX.I18N.PropertyWindow.isMappedTo],["is equal to",ORYX.I18N.PropertyWindow.isEqualTo]]})});
L.grid.getColumnModel().setEditor(L.column,new Ext.grid.GridEditor(I))
}if(i.name=="fromCombo"){i.destroy();
var J=new Ext.form.ComboBox({name:"fromCombo",valueField:"name",displayField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,store:new Ext.data.SimpleStore({fields:["name","value"],data:o})});
L.grid.getColumnModel().setEditor(L.column,new Ext.grid.GridEditor(J))
}if(i.name=="toCombo"){i.destroy();
var K=new Ext.form.ComboBox({name:"toCombo",valueField:"name",displayField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,store:new Ext.data.SimpleStore({fields:["name","value"],data:j})});
L.grid.getColumnModel().setEditor(L.column,new Ext.grid.GridEditor(K))
}}}}});
var D=new Ext.Window({layout:"anchor",autoCreate:true,title:ORYX.I18N.PropertyWindow.editorForDataAssignments,height:350,width:890,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,autoScroll:true,keys:[{key:27,fn:function(){D.hide()
}.bind(this)}],items:[a],listeners:{hide:function(){this.fireEvent("dialogClosed",this.value);
D.destroy()
}.bind(this)},buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){var i="";
a.getView().refresh();
a.stopEditing();
G.data.each(function(){if(this.data.from.length>0&&this.data.type.length>0){if(this.data.type=="is mapped to"){if(this.data.to.length>0){if(l.indexOf(this.data.from)>=0&&l.indexOf(this.data.to)>=0){if(b.indexOf(this.data.from)<0){ORYX.EDITOR._pluginFacade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"warning",msg:"Assignment for "+this.data.from+" is invalid",title:""})
}else{i+=this.data.from+"->"+this.data.to+","
}}else{if(l.indexOf(this.data.from)>=0&&j.indexOf(this.data.to)>=0){if(t.indexOf(this.data.from)<0){ORYX.EDITOR._pluginFacade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"warning",msg:"Assignment for "+this.data.from+" is invalid",title:""})
}else{i+=this.data.from+"->"+this.data.to+","
...
}else{if(t.indexOf(this.data.from)>=0&&l.indexOf(this.data.to)>=0){ORYX.EDITOR._pluginFacade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"warning",msg:"Assignment for "+this.data.from+" is invalid",title:""})
}else{i+=this.data.from+"->"+this.data.to+","
...
}}else{if(this.data.type=="is equal to"){if(this.data.tostr.length>0){if(j.indexOf(this.data.from)>=0){ORYX.EDITOR._pluginFacade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"warning",msg:"Assignment for "+this.data.from+" is invalid",title:""})
}else{var I=this.data.tostr.replace(/,/g,"##");
I=I.replace(/=/g,"||");
i+=this.data.from+"="+I+","
}}else{ORYX.EDITOR._pluginFacade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"warning",msg:"Assignment for "+this.data.from+" does not contain a proper mapping.",title:""})
}}}}});
...
}},refreshCanvasforIE:function(){if((Object.hasOwnProperty.call(window,"ActiveXObject")&&!window.ActiveXObject)||(navigator.appVersion.indexOf("MSIE 10")!==-1)){var a=this.facade.getSelection();
var e=ORYX.EDITOR.getSerializedJSON();
this.facade.setSelection(this.facade.getCanvas().getChildShapes(true));
var b=this.facade.getSelection();
var c=new ORYX.Plugins.Edit.ClipBoard();
c.refresh(b,this.getAllShapesToConsider(b,true));
var d=new ORYX.Plugins.Edit.DeleteCommand(c,this.facade);
this.facade.executeCommands([d]);
this.facade.importJSON(e);
this.facade.setSelection(a);
this.facade.getCanvas().update();
this.facade.updateSelection()
}}};
